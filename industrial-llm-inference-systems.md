...
|  | Boost Inference Performance up to 15x on NVIDIA Blackwell Using DFlash Speculative Decoding | 2026 | Speculative Decoding / Compiler / DSL | As AI systems move from single-turn interactions to coordinated multiagent workflows, low-latency inference becomes increasingly important. Autoregressive LLMs... | [primary](https://developer.nvidia.com/blog/boost-inference-performance-up-to-15x-on-nvidia-blackwell-using-dflash-speculative-decoding/) |
...
|  | Tiered KV cache for large LLMs on Amazon SageMaker HyperPod with Curvine | 2026 | KV Cache | Running large language model inference at scale forces a KV cache trade-off: oversized GPU instances or slow time-to-first-token. This post builds a tiered KV cache on Amazon SageMaker HyperPod that extends the cache into a shared, distrib… | [primary](https://aws.amazon.com/blogs/machine-learning/tiered-kv-cache-for-large-llms-on-amazon-sagemaker-hyperpod-with-curvine/) |
...
| 企业/组织 | 方案/论文 | 年份 | 对应方向 | 核心做法 | 材料 |
|---|---|---:|---|---|---|
|  | Co-Designing AI Model Attention for Fast, Interactive Long-Context Inference | 2026 | 探索观察 | As agentic and long-context workloads become common, the context lengths increase and attention consumes a larger share of inference time (Figure 1). Because... | [primary](https://developer.nvidia.com/blog/co-designing-ai-model-attention-for-fast-interactive-long-context-inference/) |
...
|  | Restore LLM Inference Capacity in Seconds with Shadow Engine Recovery in NVIDIA Dynamo | 2026 | 探索观察 | When an LLM engine process fails, the standard recovery path involves a cold restart. This requires loading weights into HBM from storage, compiling kernels,... | [primary](https://developer.nvidia.com/blog/restore-llm-inference-capacity-in-seconds-with-shadow-engine-recovery-in-nvidia-dynamo/) |
