...
`html_program` sources handle conference programs whose paper titles are rendered as plain text blocks or event-modal anchors. `html_bold_program` handles accepted-paper lists rendered as bold titles inside table/list items. URL fragments are preserved for stable per-paper identity.
